function [sample_roi_idx,roi_id,roi_name] = mm_sample2surfatlas(sample_coords,hemi,dist_thr)
%[sample_roi_idx,roi_id,roi_name] = mm_sample2surfatlas(sample_coords,hemi,dist_thr)
if nargin < 3, dist_thr = 5; end
if nargin < 2 , hemi = 'L'; end;
brainmap_dir = '/nfs/e5/stanford/ABA/brainmap';
surf_dir = fullfile(brainmap_dir,'HCP','HCP_S1200_GroupAvg_v1');
...
switch hemi
    case {'L','R'}
        [sample_roi_idx,roi_id] = sample2Benson(sample_coords,surf_dir, brainmap_dir,hemi,dist_thr);
    case 'LR'
        
        [sample_roi_idx_L,roi_id_L] = sample2Benson(sample_coords,surf_dir, brainmap_dir,'L',dist_thr);
        [sample_roi_idx_R,roi_id_R] = sample2Benson(sample_coords, surf_dir,brainmap_dir,'R',dist_thr);
...
function [sample_roi_idx,roi_id] = sample2Benson(sample_coords,surf_dir, brainmap_dir,hemi,dist_thr)
% surf file
...
label_file = fullfile(brainmap_dir,'benson_retinotopy','fs_LR',...
    sprintf('%s.benson17_varea_32k_fs_LR.func.gii',xh));

% Map samples to atlas
surf = gifti(surf_file); label = gifti(label_file);
[~,coords_roi,roi_id] = mm_coords2surf(sample_coords,surf,dist_thr,label);
