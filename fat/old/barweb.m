...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	% Plot bars
	handles.bars = bar(barvalues, width,'edgecolor','none', 'linewidth', 1);
	hold on
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', 'best', 'fontsize',FS);
		legend boxoff;
	else
		handles.legend = [];
	end
...
if strcmp(legend_type, 'axis')
    for i = 1:numbars
        ...
        for j = 1:length(xdata)
            text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60, 'fontsize', FS, 'HorizontalAlignment', 'right');
        end
    end
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',FS);
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',FS);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',FS);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', FS, 'xtick',1:numgroups, 'linewidth', 1.5,'xgrid','off','ygrid','off');
