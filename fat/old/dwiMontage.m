function bigImg = dwiMontage(dwiDir, sessid, runName, imgName)

nC = 8; % col for individual image
rows = [301,1490];
cols = [511 1970];
...
for r = 1:length(runName)
    ...
    for s = find(idx)
        ...
        if exist(fullfile(imgDir,imgName),'file')
            ...
            bigImg = addSmallImg2BigImg(bigImg,Y,[nR,nC,cnt]);
            cnt = cnt+1;
        end
    end
    ...
    % write image
    imwrite(img,fullfile('~/analysis/paperfig',sprintf('montage_%s',imgName)),'Resolution', 300);
end
